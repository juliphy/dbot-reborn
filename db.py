...
def exist_user(id,bot):
    try:
        ...
        bot.send_message(id, 'Вибачте! Щось трапилось з базою данних. Спробуйте ще раз пізніше чи напишіть до підтримки.')
...
def create_user(user,bot):
    try:
        ...
        bot.send_message(user.id, 'Вибачте! Щось трапилось з базою данних. Спробуйте ще раз пізніше чи напишіть до підтримки.')
...
def update_user(msg,bot,change_type):
    try:
        if change_type == 'name' or change_type == 'birthdate':
            ...
            bot.send_message(msg.chat.id, formatting.hitalic(change_type) + text1 + formatting.hbold(msg.text), reply_markup = Markup('only_back').markup)
        else:
