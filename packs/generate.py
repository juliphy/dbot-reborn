...
def process_face_image_step(message, bot, user):
        if message.content_type == "photo":
            ...
            msg = bot.send_message(message.chat.id,
                                   'Отправь фотографию твоей подписи примерно 270 на 110 пикселей; ' + formatting.hitalic(
                                       'Некоректная') + ' фотография подпись может остановить доступ к сайту.')
            bot.register_next_step_handler(msg, process_sign_image_step, bot, user)
        else:
